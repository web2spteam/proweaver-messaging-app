...
interface FormData {
  test_id: string | number | undefined;
  expiry_date: string;
}

const SetTestExpiryModal: FC<ModalProps> = ({ isOpen, onClose, testId}) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await postData<ApiResponse, any>("read_compre_test/setExpiryDate", formData);
      if (result.success) {
        onClose();
      } else {
...
export default SetTestExpiryModal;
