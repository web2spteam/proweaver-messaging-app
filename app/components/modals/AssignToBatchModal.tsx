...
const AssignToBatchModal: FC<ModalProps> = ({ isOpen, onClose, subjectId }) => {
    ...
    const checkIfAlreadyAssigned = async () => {
        try {
            const response = await postData<ApiResponse, any>("subjects/checkAssignment", {
    ...
    const handleAssign = async () => {
        ...
        try {
            ...
            const result = await postData<ApiResponse, any>("subjects/assignSubjectToBatch", {
                subject_id: subjectId,
                batch_id: selectedBatch,
            });
            if (result.success) {
                showMessage({
                    type: "success",
                    message: result.message || "Subject has been successfully assigned to batch.",
                });
                ...
                showMessage({
                    type: "danger",
                    message: result.message || "An error occurred while assigning subject.",
                });
            }
        } catch (error) {
    ...
    const openConfirmationModal = async () => {
        ...
        if (isAlreadyAssigned) {
            setErrorMessage("This subject is already assigned to the selected batch.");
            showMessage({
                type: "danger",
                message: "This subject is already assigned to the selected batch.",
            });
        } else {
...
export default AssignToBatchModal;
