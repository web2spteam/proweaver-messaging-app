...
const EditSubjectModal: FC<ModalProps> = ({ isOpen, onClose, subjectId }) => {
    ...
    const fetchSubjectData = useCallback(async () => {
        try {
            setLoading(true);
            const response = await getData<any>(`subjects/getSubjectInfo/${subjectId}`);
            
            // Parsing the schedule field which is in JSON format
            const schedule = JSON.parse(response.schedule);
            
            setSubjectName(response.subject_name);
            setDays(schedule.days); // Set the days
            setTimeFrom(schedule.fromTime ? convertTo24HourFormat(schedule.fromTime) : ""); // Convert time to 24-hour format
            setTimeTo(schedule.toTime ? convertTo24HourFormat(schedule.toTime) : ""); // Convert time to 24-hour format
    
        } catch (error) {
            console.error("Error fetching subject data:", error);
            setErrorMessage("Failed to load subject data.");
        } finally {
            setLoading(false);
        }
    }, [subjectId]);
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            console.error("Error occurred updating subject: ", error);
        }
    };
    ...
    const convertTo24HourFormat = (time: string) => {
        const [timePart, period] = time.split(" "); // Split time from AM/PM
