...
const EditTest: React.FC = () => {
    ...
    const [moduleID, setModuleID] = useState<string | number | undefined>(undefined);
    const [testID, setTestID] = useState<string | number | string[]>(test_id);
    ...
    const [items, setItems] = useState<TestItem[]>([]); // Initialize items as an empty array
    ...
    const fetchTestData = useCallback(async () => {
        ...
        try {
            const res: any = await getData(
                `read_compre_test/getTestData/${test_id}`,
            );
            if (res.success) {
                setTestTitle(res.test_title);
                setTestText(res.test_text);
                setItems(res.questions);
                setTestStatus(res.test_status);
                setModuleID(res.module_id);
            } else {
                console.error(res.msg);
                showMessage({ type: "danger", message: res.msg || "Failed to load test data." });
            }
        } catch (error) {
            console.error("An error occurred fetching test data:", error);
            showMessage({ type: "danger", message: "An unexpected error occurred while fetching test data." });
        } finally {
    ...
    useEffect(() => {
        const savedDesciption = localStorage.getItem("testText");
        if (savedDesciption) {
            setTestText(savedDesciption);
        }
    }, []);
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="col-span-3 md:col-span-2">
                            {items.length === 0 && !isLoading ? ( // Conditional rendering for empty items
                                <p>No items found for this test.</p>
                            ) : (
